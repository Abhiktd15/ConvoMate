...
const HomePage = () => {
  ...
  const {data:outGoindFriendReqs} = useQuery({
  ...
  useEffect(() => {
    ...
    if(outGoindFriendReqs && outGoindFriendReqs.length > 0){
      outGoindFriendReqs.forEach((req) => {
        outGoingIds.add(req.recipient._id)
      })
      setOutGoingReqs(outGoingIds)
    }
  },[outGoindFriendReqs])
          ...
          {loadingUsers ? (
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'> 
              {recommendedUsers?.map((user) => {
                const hasRequestBeenSent = outGoingReqs.has(user._id)
                return (
